...
import { IoCloseOutline, IoLogoGoogle, IoLogoFacebook, IoMailOutline, IoLockClosedOutline, IoPersonOutline } from "react-icons/io5";
import { useAuth } from "../context/AuthContext";
import { useNavigate } from "react-router-dom";

const AuthModal = () => {
    const { authModalMode, openAuthModal, closeAuthModal, login, signup } = useAuth();
                    ...
                    <div className="hidden md:flex flex-col justify-center w-[45%] bg-brand p-12 text-white relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <h2 className="font-heading text-4xl font-bold mb-4 leading-tight">
                                {isLogin ? "Elevate your shopping experience." : "Join our fashion community."}
                            </h2>
                    ...
                    <div className="flex-1 p-8 md:p-12 flex flex-col justify-center bg-white relative">
                        <div className="w-full max-w-sm mx-auto">
                            <div className="text-center mb-8">
                                <h1 className="font-heading text-3xl font-extrabold text-brand tracking-tight mb-4">NEHDO</h1>
                                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                                    {isLogin ? "Welcome Back" : "Create an Account"}
                                </h2>
                                <p className="text-sm text-gray-500">
                                    {isLogin ? "Please login to your account" : "Enter your details to register"}
                                </p>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                {isLogin && (
                                    <div className="flex justify-end pt-1">
                                        <button type="button" className="text-xs font-semibold text-gray-500 hover:text-brand transition-colors">
                                            Forgot password?
                                        </button>
                                    </div>
                                )}
                            ...
                            <div className="mt-8">
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-6">
                                    <button className="flex items-center justify-center gap-2 py-2.5 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
                                        <IoLogoGoogle size={18} className="text-[#DB4437]" />
                                        <span className="text-sm font-semibold text-gray-700">Google</span>
                                    </button>
                                    <button className="flex items-center justify-center gap-2 py-2.5 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
                                        <IoLogoFacebook size={18} className="text-[#4267B2]" />
