...
import { IoCheckmarkCircle, IoCardOutline, IoLocationOutline, IoLockClosedOutline, IoArrowForwardOutline } from "react-icons/io5";
import { useCart } from "../context/CartContext";
import { useAuth } from "../context/AuthContext"
import Breadcrumb from "../components/Breadcrumb";
import { userapiRequest } from "../services/apiService"
const Checkout = () => {
    const { items, subtotal, shipping, total, clearCart, setShippingCharge } = useCart();
    const { user } = useAuth();
    ...
    // User name mathi first/last name
    ...
    const handleRazorpayPayment = async () => {
        try {
            ...
            if (!cartIds.length) {
                console.error("Cart IDs not found");
                return;
            }
            ...
            const result = await userapiRequest(
                ...
                "POST",
                { cartIds,discount,shippingcharge:shipping }
            );
            ...
            const options = {
                ...
                handler: async function (response: any) {
                    ...
                    try {
                        ...
                        // 2. Verify Payment
                        const verifyResponse = await userapiRequest(
                        ...
                        // 3. Only after backend verification
                        ...
                        <AnimatePresence mode="wait">
                            {step === 1 && (
                                ...
                                >
                                    ...
                                    <input
                                        type="text"
                                        value={defaultAddress?.addressline + " " + defaultAddress.landmark || ""}
                                        readOnly
                                        className="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 cursor-not-allowed mb-4"
                                    />
                                ...
                                >
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => setStep(1)}
                                            className="px-6 py-4 rounded-xl font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 transition-all"
                                        >
                                            Back
                                        </button>
                                        ...
                                        <button
                                            ...
                                            className="flex-1 py-4 bg-brand text-white font-bold rounded-xl shadow-button hover:bg-brand-light transition-all"
                                        >
                            ...
                            {step === 3 && (
                                <motion.form key="s3" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} onSubmit={handlePlaceOrder} className="bg-white rounded-3xl p-6 md:p-8 shadow-card text-center">
                                    ...
                                    <div className="text-left bg-gray-50 p-6 rounded-2xl mb-8">
                                        ...
                                        <p className="text-sm text-gray-600 leading-relaxed">{firstName} {lastName}<br />{defaultAddress?.addressline + " " + defaultAddress.landmark || ""}<br />{defaultAddress.city}, {defaultAddress.state} {defaultAddress.postalCode}</p>
                                    </div>
