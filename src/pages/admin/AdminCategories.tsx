...
import {
    ...
    IoCloseOutline, IoCloudUploadOutline, IoPencilOutline,
...
const AdminCategories = () => {
    ...
    const fetchCategories = async () => {
        try {
            ...
            const res = await apiRequest(
                "/admin/api/v1/category/get",
                "GET"
            );

            const mainFormatted = (res.categories || []).map((item: any) => ({
                id: item._id,
                name: item.name,
                slug: item.slug || "",
                parentCategory: null,
                ...
                showOnHome: item.homepageDisplay || false,
                showInNav: item.navbarmenuDisplay || false,
            }));

            // Sub categories
            const subFormatted = await fetchSubCategories();

            // Merge both
            setCategoryList([...mainFormatted, ...subFormatted]);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const fetchSubCategories = async () => {
        try {
            const res = await apiRequest("/admin/api/v1/subcategory/get","GET");
            return (res.subcategories || []).map((item: any) => ({
                id: item._id,
                name: item.name,
                slug: item.slug || "",
    ...
    const handleSaveCategory = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (categoryType === "sub") {
                ...
                formData.append("categoryId", String(selectedParent.id));
                ...
                formData.append("homepageDisplay", String(showOnHome));
                formData.append("navbarmenuDisplay", String(showInNav));

                if (logoFile) {
                    formData.append("subcategorylogo", logoFile);
                }
            ...
            if (logoFile) {
                formData.append("categorylogo", logoFile);
            }

            if (editingCategory) {
                await apiRequest(`/admin/api/v1/category/update/${editingCategory.id}`, "PATCH", formData);
            } else {
                await apiRequest("/admin/api/v1/category/add", "POST", formData,);
            }
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <button
                                            ...
                                            className={`p-3 rounded-xl border flex items-center justify-center gap-2 text-sm font-semibold transition-all ${categoryType === "main"
                                        ...
                                        <button
                                            ...
                                            className={`p-3 rounded-xl border flex items-center justify-center gap-2 text-sm font-semibold transition-all ${categoryType === "sub"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="md:col-span-2">
                                        ...
                                        <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center bg-gray-50 hover:bg-gray-100 transition-colors">
                                            ...
                                            <label htmlFor="categoryLogo" className="cursor-pointer block">
                                                ...
                                                {logoPreview ? (
                                                    <div className="space-y-3">
                                                        <img
                                                            ...
                                                            className="mx-auto h-32 w-32 object-cover rounded-lg border border-gray-200 shadow-sm"
                                                        />
                                                    ...
                                                    <div className="space-y-2">
                                                        ...
                                                        <p className="text-xs text-gray-400 mt-1">
                                                            Recommended: 800×400px, PNG, JPG or WEBP
                                                        </p>
                                                    </div>
                                                )}
                    ...
                    <div className="flex p-1 bg-gray-100 rounded-xl w-full sm:w-auto">
                        {(["All", "Main", "Sub"] as const).map(type => (
                            <button
                                ...
                                className={`flex-1 sm:flex-none px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${filterType === type
                                ...
                                filteredCategories.map((category) => {
                                    ...
                                    return (
                                        <tr key={category.id} className="hover:bg-gray-50/80 transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="w-10 h-10 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden flex items-center justify-center">
                                                    {category.image ? (
                                                        ...
                                                        <IoGitBranchOutline size={18} className="text-purple-500" />
                                                    ) : (
                                                        <IoFolderOutline size={18} className="text-brand" />
                                                    )}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${category.status === "Active"
