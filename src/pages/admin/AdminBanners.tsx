...
const AdminBanners = () => {
    ...
    const [formData, setFormData] = useState<BannerForm>(createInitialForm());
    ...
    const fetchBanners = async () => {
        try {
            ...
            const response = await apiRequest("/admin/api/v1/banner/get", "GET");

            if (response?.success) {
                const formattedBanners: Banner[] = (response.banners || []).map(
                    (item: any) => ({
                        id: item._id,
                        title: item.title || "",
    ...
    const handlePlacementChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        setFormData((prev) => ({
            ...prev,
            placement,
            ctaButtonText: placement === "Hero Slider" ? "" : prev.ctaButtonText,
            productSku: placement === "Promotional Strip" ? "" : prev.productSku,
        }));
    };
    ...
    const handleSave = async () => {
        try {
            ...
            if (!isEditing && !formData.desktopImageFile) {
                toast.warning("Please select desktop image.");
                return;
            }


            if (formData.placement === "Hero Slider" && !formData.productSku.trim()) {
                toast.warning("Please enter product SKU.");
                return;
            }
            ...
            if (formData.placement === "Promotional Strip") {
                payload.append("ctaButtonText", formData.ctaButtonText.trim());
            }

            if (formData.placement === "Hero Slider") {
                payload.append("productSku", formData.productSku.trim());
            }

            if (formData.desktopImageFile) {
                payload.append("desktopImage", formData.desktopImageFile);
            }

            if (formData.mobileImageFile) {
                payload.append("mobileImage", formData.mobileImageFile);
            }

            console.log("FORM DATA:", Array.from(payload.entries()));


            if (!isEditing) {
                const response = await apiRequest("/admin/api/v1/banner/add", "POST", payload);
            ...
            const response = await apiRequest(`/admin/api/v1/banner/update/${editingBannerId}`, "PATCH", payload,);
    ...
    const filteredBanners = banners.filter((banner) => {
        ...
        return (
            banner.title.toLowerCase().includes(searchValue) || banner.placement.toLowerCase().includes(searchValue) || (banner.productSku || "").toLowerCase().includes(searchValue)
        );
    });
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10">
                            <h2 className="text-xl font-bold text-gray-900">{isEditing ? "Edit Banner" : "Add New Banner"}</h2>

                            <button onClick={closeModal} disabled={isLoading} className="p-2 text-gray-400 hover:text-gray-900 rounded-full hover:bg-gray-100" >
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input ref={desktopInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => handleImageChange(e, "desktop")} />

                                    {formData.desktopImage ? (
                                        <div className="relative border border-gray-200 rounded-xl overflow-hidden bg-gray-50">
                                            <img src={formData.desktopImage} alt="Desktop preview" className="w-full h-40 object-cover" />
                                        ...
                                        <div
                                            ...
                                            <p className="text-xs text-gray-400 mt-1">1920×800px recommended</p>
                                        </div>
                                    )}
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-2">Mobile Image</label>
                                    <input ref={mobileInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => handleImageChange(e, "mobile")} />

                                    {formData.mobileImage ? (
                                        <div className="relative border border-gray-200 rounded-xl overflow-hidden bg-gray-50">
                                            <img src={formData.mobileImage} alt="Mobile preview" className="w-full h-40 object-cover" />
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="e.g. Summer Sale 2026"
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <input type="text" name="subtitle" value={formData.subtitle} onChange={handleChange} placeholder="e.g. Up to 50% off"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-brand" />
                                </div>

                                {/* CTA */}

                                {formData.placement === "Promotional Strip" && (
                                    <div className="md:col-span-2">
                                        <label className="block text-sm font-semibold text-gray-700 mb-2"> CTA Button Text </label>
                                        <input type="text" name="ctaButtonText" value={formData.ctaButtonText} onChange={handleChange} placeholder="e.g. Shop Now"
                                            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-brand" />
                                    </div>
                                )}
                                ...
                                {formData.placement === "Hero Slider" && (
                                    <div className="md:col-span-2">
                                        ...
                                        <input type="text" name="productSku" value={formData.productSku} onChange={handleChange} placeholder="e.g. PROD-001"
                                            className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-brand" />
                                        <p className="text-xs text-gray-400 mt-1"> Enter the SKU of the product linked to this hero slider. </p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select value={formData.placement} onChange={handlePlacementChange}
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-brand" >
                                        <option value="Promotional Strip">Promotional Strip</option>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="priority" value={formData.priority} onChange={handleChange} placeholder="1" min="0"
                                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:border-brand" />
                                </div>
                                <div>
                        ...
                        <div className="p-6 border-t border-gray-100 flex justify-end gap-3 sticky bottom-0 bg-white z-10">
                            <button onClick={closeModal} disabled={isLoading} className="px-6 py-2.5 rounded-xl border border-gray-200 text-sm font-bold text-gray-600 hover:bg-gray-50 disabled:opacity-50">
                                Cancel
                            </button>

                            <button onClick={handleSave} disabled={isLoading} className="px-6 py-2.5 rounded-xl bg-brand text-white text-sm font-bold hover:bg-brand-light shadow-sm disabled:opacity-50" >
                                {isLoading ? "Saving..." : isEditing ? "Update Banner" : "Save Banner"}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-4 border-b border-gray-100">
                    <div className="relative w-full sm:w-72">
                        ...
                        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search banners..."
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="bg-gray-50 text-gray-500 font-semibold uppercase text-xs tracking-wider">
                                <tr>
                                    ...
                                    <th className="px-6 py-4">CTA / Product SKU</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                    ...
                                    <tr key={banner.id} className="hover:bg-gray-50 transition-colors" >
                                        <td className="px-6 py-4">
                                            <div className="w-24 h-12 bg-gray-200 rounded overflow-hidden flex items-center justify-center text-gray-400">
                                                {banner.desktopImage ? (<img src={banner.desktopImage} alt={banner.title} className="w-full h-full object-cover" />) : (<IoImageOutline size={20} />)}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-gray-500 text-xs">
                                            {banner.placement === "Hero Slider" ? banner.productSku || "No Product SKU" : banner.ctaButtonText || "No CTA"}
                                        </td>
                                        <td className="px-6 py-4 text-gray-500 text-xs">{getScheduleText(banner)}</td>
                                        <td className="px-6 py-4 text-gray-600">{banner.priority}</td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold
                                            ${banner.status === "Active" ? "bg-green-100 text-green-700" : banner.status === "Scheduled" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600"}`}>
                                                {banner.status}
                                            </span>
                                        </td>

                                        <td className="px-6 py-4 text-right">
                                            <button onClick={() => openEditModal(banner)} className="text-brand text-sm font-semibold hover:underline mr-3" > Edit </button>
                                ...
                                {filteredBanners.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="px-6 py-10 text-center text-sm text-gray-500" > No banners found. </td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
